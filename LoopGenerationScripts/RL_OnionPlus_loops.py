...
def RL_onionPlus(N):
	...
	for i in range(2,N,2):
		loops = generateOnionLayerNPlus(N-i)
		loops = helper.shiftAllLoops(loops, shift, shift)
		L = L + loops
		shift += 1 
		
	L += generateOnionLayerNPlus(N)
	...
	for i in range(N//4):
		...
		ul = x + 1
		lr = N**2 - x - 2
		l = helper.generateLoop_2D_coordinates(ul//N, ul%N, lr//N, lr%N)
		l.reverse()
		L.append(l)
...
HP_RL_StatsCSV = open("OINIONPlus_Stats.csv", "w")
...
for N in [4,6,8,10,12,14,16]:
	...
	routingTable.RoutingTableToFile(N, L, f"RLOnionPlus{N}.txt")
